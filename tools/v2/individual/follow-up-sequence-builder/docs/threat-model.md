# Follow-up Sequence Builder — Threat Model and Hardening Notes

This document records the security assumptions for the isolated Follow-up
Sequence Builder tool and the guards that enforce them. It covers this folder
only and does not describe the wider mail application.

## Trust boundary

- The sequence engine is a pure, local, rule-based text scanner.
...
- It is read-only: it returns a sequence plan and never sends, saves, or
  mutates anything. The engine does not send email, create calendar events,
...
| Very high word count in body              | Heavy work in signal scanning passes              | Word cap rejects early; engine scanning is bounded by MAX_SCAN_LENGTH  |
...
| Massive existingSequences array           | Excessive iteration, degraded dedup performance   | validateOptions clamps the array; checkOptionsLimits rejects oversized |
| Malformed existingSequence entries        | Type errors in dedup logic                        | validateOptions filters out entries with missing or invalid fields     |
| Ambiguous or contradictory signals        | Incorrect urgency or step assignment              | Engine handles weak signals by returning low confidence with warnings  |
...
- Protection against UI-side attacks (XSS, clickjacking, etc.) — these belong
  to the integration layer that renders the sequence model.
